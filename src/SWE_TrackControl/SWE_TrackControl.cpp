...
cSWE_TrackControl::cSWE_TrackControl(const tChar* __info):cFilter(__info)
{
...
//TODO:
        ...
        RETURN_IF_FAILED(m_VecLaneMarkings.Create("VecLaneMarkings", pInputType, this));
        RETURN_IF_FAILED(RegisterPin(&m_VecLaneMarkings));
    ...
    if (nEventCode == IPinEventSink::PE_MediaSampleReceived)
    {
        ...
        if (pSource == &m_VecLaneMarkings)
        {
            ...
            if (IS_OK(AllocMediaSample(&pNewSample)))
            {
                ...
                m_SteeringAngle.Transmit(pNewSample);
            }
        }
    }
